...
/// Migration to re-key TokenGateway storage maps that use AssetId as a Blake2_128Concat key.
///
/// Affected maps:
///   SupportedAssets : Blake2_128Concat AssetId → H256       (simple map, 52→64 bytes)
///   NativeAssets    : Blake2_128Concat AssetId → bool       (simple map, 52→64 bytes)
///   Precisions      : Blake2_128Concat AssetId, Blake2_128Concat StateMachine → u8
///                     (double map, variable second key length)
const TOKEN_GATEWAY_KEY_MIGRATION_FROM_SPEC: u32 = 382;

pub struct TokenGatewayStorageKeyMigration;

impl OnRuntimeUpgrade for TokenGatewayStorageKeyMigration {
    fn on_runtime_upgrade() -> Weight {
        if crate::System::last_runtime_upgrade_spec_version() != TOKEN_GATEWAY_KEY_MIGRATION_FROM_SPEC {
            log::info!("Skipping TokenGatewayStorageKeyMigration");
            return <crate::Runtime as frame_system::Config>::DbWeight::get().reads(1);
        }
        ...
        // Simple maps: old key = 52 bytes, new key = 64 bytes
        for storage_name in &[b"SupportedAssets" as &[u8], b"NativeAssets"] {
            let (r, w) = rekey_assets_map(b"TokenGateway", storage_name, 52);
            total_reads += r;
            total_writes += w;
        }

        // Precisions double map: variable-length StateMachine second key
        // old first-key part = blake2_128(u32)(16) + u32(4) = 20 bytes at [32..52]
        // new first-key part = blake2_128(u128)(16) + u128(16) = 32 bytes at [32..64]
        let (r, w) = rekey_token_gateway_precisions();
        total_reads += r;
        total_writes += w;

        log::info!(
            "🔧 TokenGatewayStorageKeyMigration complete: {} reads, {} writes",
            total_reads, total_writes
        );
        <crate::Runtime as frame_system::Config>::DbWeight::get()
            .reads_writes(total_reads, total_writes)
    ...
    fn pre_upgrade() -> Result<sp_std::vec::Vec<u8>, sp_runtime::TryRuntimeError> {
        use sp_io::hashing::twox_128;
        let mut total = 0u32;
        for storage_name in &[b"SupportedAssets" as &[u8], b"NativeAssets", b"Precisions"] {
            let prefix: sp_std::vec::Vec<u8> = [
                twox_128(b"TokenGateway").as_ref(),
                twox_128(storage_name).as_ref(),
            ].concat();
            let mut key = prefix.clone();
            loop {
        ...
        log::info!("🔍 TokenGatewayStorageKeyMigration pre-upgrade: {} entries", total);
        Ok(total.encode())
    }
    ...
    fn post_upgrade(state: sp_std::vec::Vec<u8>) -> Result<(), sp_runtime::TryRuntimeError> {
        ...
        ensure!(pre_count == post_count, "TokenGatewayStorageKeyMigration: entry count mismatch");
        log::info!("✅ TokenGatewayStorageKeyMigration post-upgrade: {} entries re-keyed", post_count);
        Ok(())
    }
}

/// Re-keys `TokenGateway::Precisions` (double map) from u32 to u128 AssetId first key.
///
/// Uses explicit old-key prefix search per known AssetId since the StateMachine
/// second key is variable length and prevents key-length-based detection.
fn rekey_token_gateway_precisions() -> (u64, u64) {
    use sp_io::hashing::{twox_128, blake2_128};
    use sp_runtime::codec::Encode;

    let prefix: sp_std::vec::Vec<u8> = [
        twox_128(b"TokenGateway").as_ref(),
        twox_128(b"Precisions").as_ref(),
    ].concat();

    let mut reads = 0u64;
    let mut writes = 0u64;

    // Iterate plausible old u32 AssetId values (chain only has a handful)
    for old_id_u32 in 0u32..=255u32 {
        let old_encoded = old_id_u32.encode();
        let old_hash = blake2_128(&old_encoded);

        // Old first-key prefix: storage_prefix + blake2_128(u32) + u32
        let mut old_key_prefix = prefix.clone();
        old_key_prefix.extend_from_slice(&old_hash);
        old_key_prefix.extend_from_slice(&old_encoded);

        // Collect all entries under this old first-key prefix
        let mut old_entries: sp_std::vec::Vec<sp_std::vec::Vec<u8>> = sp_std::vec::Vec::new();
        let mut cursor = old_key_prefix.clone();
        loop {
            reads += 1;
            match sp_io::storage::next_key(&cursor) {
                Some(next) if next.starts_with(&old_key_prefix) => {
                    old_entries.push(next.clone());
                    cursor = next;
                }
                _ => break,
            }
        ...
        let new_id: u128 = old_id_u32 as u128;
        let new_encoded = new_id.encode();
        let new_hash = blake2_128(&new_encoded);

        for old_key in old_entries {
            reads += 1;
            if let Some(value) = sp_io::storage::get(&old_key) {
                // StateMachine part: everything after the old first-key (32+16+4=52 bytes)
                let state_machine_part = &old_key[52..];

                let mut new_key = prefix.clone();
                new_key.extend_from_slice(&new_hash);
                new_key.extend_from_slice(&new_encoded);
                new_key.extend_from_slice(state_machine_part);
