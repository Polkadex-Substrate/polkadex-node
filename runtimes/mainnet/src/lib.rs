...
// --------------- imports starts
//use crate::runtime::{
//    System, Utility, Babe, Timestamp, Authorship, Indices, Balances, TransactionPayment, ElectionProviderMultiPhase,
//    Staking, Session, Council, TechnicalCommittee, Elections, TechnicalMembership,
//    Grandpa, Treasury, Sudo, ImOnline, AuthorityDiscovery, Offences, Historical, Identity, Recovery,
//    Scheduler, Proxy, Multisig, Bounties, Democracy, Preimage, ChildBounties, Assets, PoolAssets,
//    Rewards, AssetConversion, AssetConversionTxPayment, Statement, AssetTxPayment, OrderbookCommittee,
//    CrowdSourceLMP, PDEXMigration, OCEX, Thea
//};
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    ...
    impl_name: alloc::borrow::Cow::Borrowed("polkadex-official"),
...
pub const BABE_GENESIS_EPOCH_CONFIG: sp_consensus_babe::BabeEpochConfiguration = sp_consensus_babe::BabeEpochConfiguration {
...
//type ImbalanceType = frame_support::traits::tokens::fungible::Imbalance<
//    u128,
//		DecreaseIssuance<AccountId, pallet_balances::Pallet<Runtime>>,
//		IncreaseIssuance<AccountId, pallet_balances::Pallet<Runtime>>,
//>;
...
parameter_types! {
    ...
    pub ElectionBoundsOnChain: ElectionBounds = ElectionBoundsBuilder::default().voters_count(5_000.into()).targets_count(1_250.into()).build();
    ...
    pub MinerMaxLength: u32 = Perbill::from_rational(9u32, 10) * *RuntimeBlockLength::get().max.get(DispatchClass::Normal);
    // miner configs
    ...
    pub ElectionBoundsOld: ElectionBounds = ElectionBoundsBuilder::default().voters_count(MaxElectingVoters::get().into()).build();
    ...
    pub const UsernameDeposit: Balance = deposit(1, 32);
    pub const SubAccountDeposit: Balance = deposit(1,53);   // 53 bytes on-chain
    pub const MaxSubAccounts: u32 = 100;
    pub const MaxRegistrars: u32 = 20;
    pub const PendingUsernameExpiration: BlockNumber = 1000u32;
    pub const UsernameGracePeriod: BlockNumber = 500u32;
    pub const MaxSuffixLength: u32 = 8;
    ...
    //pub const WestendLocation: xcm::v5::Location = xcm::v5::Location::parent();
...
//impl OnUnbalanced<ImbalanceType> for DealWithFees {
//    fn on_unbalanceds(mut fees_then_tips: impl Iterator<Item = ImbalanceType>) {
//        if let Some(mut fees) = fees_then_tips.next() {
//            if let Some(tips) = fees_then_tips.next() {
//                tips.merge_into(&mut fees);
//                //tips.try_merge(&mut fees);
//            }
...
impl pallet_beefy::Config for Runtime {
	...
	type WeightInfo = ();
	type KeyOwnerProof = sp_session::MembershipProof;
	type EquivocationReportSystem = pallet_beefy::EquivocationReportSystem<Self, Offences, Historical, ReportLongevity>;
}
...
//	//type RuntimeHoldReason = pallet_staking::HoldReason;
...
//	fn create_inherent(call: RuntimeCall) -> <Block as BlockT>::Extrinsic {
//		<Block as BlockT>::Extrinsic::new_bare(call)
//	}
//}
...
//
//	fn get() -> Self::Type {
//		TreasuryPalletId.into_account_truncating()
//	}
//}
...
//impl<C> frame_system::offchain::CreateTrasactionBase<C> for Runtime
//where
//	RuntimeCall: From<C>,
//{
//	type Extrinsic = UncheckedExtrinsic;
...
impl pallet_identity::Config for Runtime {
	...
	//type OffchainSignature = sp_core::ed25519::Signature;
	type OffchainSignature = sp_runtime::MultiSignature;
	//type SigningPublicKey = sp_core::ed25519::Public;
	type SigningPublicKey = <sp_runtime::MultiSignature as sp_runtime::traits::Verify>::Signer;
...
//impl frame_system::offchain::CreateTransactionBase<pallet_ocex_lmp::Call<Runtime>> for Runtime {
//	type Extrinsic = <Block as BlockT>::Extrinsic;
//	type RuntimeCall = RuntimeCall;
//}
...
//impl frame_system::offchain::CreateTransactionBase<thea::Call<Runtime>> for Runtime {
//	type Extrinsic = <Block as BlockT>::Extrinsic;
//	type RuntimeCall = RuntimeCall;
//}
//
...
//	//type OnChargeAssetTransaction = pallet_asset_conversion_tx_payment::SwapAssetAdapter<u128, Balances, u128, ()>;
//  type OnChargeAssetTransaction = pallet_asset_conversion_tx_payment::SwapAssetAdapter<
//		WestendLocation,
//		NativeAndNonPoolAssets,
//		AssetConversion,
//		frame_support::traits::tokens::imbalance::ResolveAssetTo<StakingPot, NativeAndNonPoolAssets>,
//	>;
//	type WeightInfo = pallet_asset_conversion_tx_payment::weights::SubstrateWeight<Runtime>;
//}
...
// pallet_assets: Instance1 config implementation
// https://github.com/paritytech/polkadot-sdk/blob/1530a8826416514c3326597338b3511a55040663/substrate/bin/node/runtime/src/lib.rs#L1812
...
// https://github.com/paritytech/polkadot-sdk/blob/1530a8826416514c3326597338b3511a55040663/substrate/bin/node/runtime/src/lib.rs#L1847C1-L1848C1
...
//		Democracy: pallet_democracy = 28,
//		Preimage: pallet_preimage = 29,
//		ChildBounties: pallet_child_bounties = 30,
//		Assets: pallet_assets::<Instance1> = 31,
//		PoolAssets: pallet_assets::<Instance2> = 32,
//		AssetConversion: pallet_asset_conversion = 33,
//		AssetConversionTxPayment: pallet_asset_conversion_tx_payment = 34,
//		Statement: pallet_statement = 35,
//		Revive: pallet_revive::pallet = 36,
//		SkipFeelessPayment: pallet_skip_feeless_payment = 37,
//		Contracts: pallet_contracts = 38,
//		Alliance: pallet_alliance = 39,
//		AllianceMotion: pallet_collective::<Instance3> = 40,
//		NominationPools: pallet_nomination_pools = 41,
//		DelegatedStaking: pallet_delegated_staking = 42,
//		RandomnessCollectiveFlip: pallet_insecure_randomness_collective_flip = 43,
//		SafeMode: pallet_safe_mode = 44,
//		TxPause: pallet_tx_pause = 45,
//		MultiBlockMigrations: pallet_migrations = 46,
//		Beefy: pallet_beefy = 47,
//		Mmr: pallet_mmr = 48,
//		MmrLeaf: pallet_beefy_mmr = 49,
//		Mixnet: pallet_mixnet = 50,
//		Society: pallet_society = 51,
//		// Custom
//		OCEX: pallet_ocex_lmp = 52,
//		CrowdSourceLMP: pallet_lmp::pallet = 53,
//		Rewards: pallet_rewards = 54,
//		PDEXMigration: pdex_migration::pallet = 55,
//		OrderbookCommittee: pallet_collective = 56,
//		//Thea: thea
//	}
//);
...
//    Assets: pallet_assets::<Instance1> = 34,
//    PoolAssets: pallet_assets::<Instance2> = 35,
//		Rewards: pallet_rewards = 39,
//		//TheaExecutor: thea_executor::pallet = 44,
//		AssetConversion: pallet_asset_conversion = 40,
//		AssetConversionTxPayment: pallet_asset_conversion_tx_payment = 41,
//		Statement: pallet_statement = 42,
//		//AssetTxPayment: pallet_asset_tx_payment = 43,
//		OrderbookCommittee: pallet_collective::<Instance3> = 37,
//    // custom
//		CrowdSourceLMP: pallet_lmp::pallet = 44,
//		PDEXMigration: pdex_migration::pallet = 29,
//		OCEX: pallet_ocex_lmp = 36,
//		//Thea: thea::pallet = 38,
//    //ForeignAssets: pallet_assets = 51,
//    //CollatorSelection: pallet_collator_selection = 52,
//	}
//);
//
//#[cfg(feature = "runtime-benchmarks")]
...
type Migrations = (
    ...
    migrations::ClearIsmpStaleRequests,
);
		...
		fn account_balances(assets : Vec<u128>, account_id : AccountId) ->  Vec<u128> {
			assets
				...
				.map(|asset| {
					...
					Assets::balance((*asset).try_into().unwrap(), &account_id).saturated_into()
					}else{
