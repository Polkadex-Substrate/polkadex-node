	...
	fn account_balances(
		...
		let assets: RpcResult<Vec<_>> = assets
			...
			.map(|asset_id| {
				asset_id
					.parse::<u128>()
					.map_err(|e| ErrorObject::owned(RUNTIME_ERROR, "Runtime error", Some(format!("{e:?}"))))
			})
			.collect();
		...
		//runtime_api_result
		//	.map(|balances| balances.iter().map(|balance| balance).collect())
		//	.map_err(|e| ErrorObject::owned(RUNTIME_ERROR, "Runtime error", Some(format!("{e:?}"))))?
		runtime_api_result
            .map(|balances| {
                balances
...
//	JsonRpseeError::Custom(ErrorObject::owned(RUNTIME_ERROR, "Runtime error", Some(format!("{err:?}"))))
//		.into()
//}
