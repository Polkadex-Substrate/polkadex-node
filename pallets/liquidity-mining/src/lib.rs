...
pub mod pallet {
	...
	use frame_system::{
		...
		//offchain::{CreateTransactionBase, SubmitTransaction, SendUnsignedTransaction},
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn register_pool(
			...
			T::OtherAssets::create(share_id.try_into().unwrap(), pool.clone(), true, One::one())?;
			...
			if let Some(base_asset) = market.base.asset_id() {
				T::OtherAssets::transfer(
					...
					&market_maker,
					&pool,
					T::OtherAssets::minimum_balance(base_asset.try_into().unwrap()),
			...
			if let Some(quote_asset) = market.quote.asset_id() {
				T::OtherAssets::transfer(
					...
					&market_maker,
					&pool,
					T::OtherAssets::minimum_balance(quote_asset.try_into().unwrap()),
					Preservation::Preserve,
				)?;
			}
			T::OtherAssets::transfer(
		...
		pub fn add_liquidity(
			...
			let total_shares_issued = Decimal::from(
				T::OtherAssets::total_issuance(config.share_id.try_into().unwrap()).saturated_into::<u128>(),
			)
		...
		pub fn remove_liquidity(
			...
			let burned_amt = T::OtherAssets::burn_from(
				config.share_id.try_into().unwrap(),
				&lp,
				shares,
		...
		pub fn claim_force_closed_pool_funds(
			...
			let lp_shares = T::OtherAssets::reducible_balance(
				pool_config.share_id.try_into().unwrap(),
				&lp,
				Preservation::Expendable,
				Fortitude::Force,
			);
			let total_issuance = T::OtherAssets::total_issuance(pool_config.share_id.try_into().unwrap());
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn take_snapshot() {
			...
			let _call: pallet::Call<T> = Call::submit_scores_of_lps { results };
			log::info!(target:"liquidity-mining","Unable to submit unsigned transaction");
